...
pub fn get_inner_dialect(data_source: &DataSource) -> Box<dyn InnerDialect> {
    match data_source {
        ...
        DataSource::Teradata => Box::new(TeradataDialect {}),
        DataSource::Hive => Box::new(HiveDialect {}),
        DataSource::Impala => Box::new(ImpalaDialect {}),
...
/// [TeradataDialect] is a dialect that handles Teradata-specific SQL generation.
pub struct TeradataDialect {}

impl InnerDialect for TeradataDialect {
    fn scalar_function_to_sql_overrides(
        &self,
        unparser: &Unparser,
        function_name: &str,
...
/// [HiveDialect] is a dialect that handles Hive-specific SQL generation.
pub struct HiveDialect {}

impl InnerDialect for HiveDialect {
    fn scalar_function_to_sql_overrides(
        &self,
        unparser: &Unparser,
        function_name: &str,
        ...
        match function_name {
            "date_part" => {
                ...
                | Expr::Literal(ScalarValue::LargeUtf8(Some(field)), _) = &args[0]
                {
                    let hive_func = match field.to_lowercase().as_str() {
                        "year" => "year",
                        "month" => "month",
                        "day" => "day",
                        "hour" => "hour",
                        ...
                        _ => {
                            return plan_err!(
                                "Unsupported date part '{}' for Hive. Supported values: \
...
/// [ImpalaDialect] is a dialect that handles Impala-specific SQL generation.
pub struct ImpalaDialect {}

impl InnerDialect for ImpalaDialect {
    fn scalar_function_to_sql_overrides(
        &self,
        unparser: &Unparser,
        function_name: &str,
        ...
        match function_name {
            "date_part" => {
                ...
                | Expr::Literal(ScalarValue::LargeUtf8(Some(field)), _) = &args[0]
                {
                    let impala_func = match field.to_lowercase().as_str() {
                        "year" => "year",
                        "month" => "month",
                        "day" => "day",
                        "hour" => "hour",
                        ...
                        _ => {
                            return plan_err!(
                                "Unsupported date part '{}' for Impala. Supported values: \
                                 year, month, day, hour, minute, second, week",
                                field
                            )
                        }
