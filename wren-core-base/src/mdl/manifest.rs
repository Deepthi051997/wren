...
impl Display for DataSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DataSource::Teradata => write!(f, "TERADATA"),
            DataSource::Hive => write!(f, "HIVE"),
            DataSource::Impala => write!(f, "IMPALA"),
        }
    }
}
...
impl FromStr for DataSource {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_uppercase().as_str() {
            ...
            "TERADATA" => Ok(DataSource::Teradata),
            "HIVE" => Ok(DataSource::Hive),
            "IMPALA" => Ok(DataSource::Impala),
